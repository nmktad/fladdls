...
fl_algorithm = fed_config.get("algorithm", "fedavg")
fedprox_mu = fed_config.get("fedprox_mu", 0.01)
...
def serve():
    ...
    fl_server.set_training_function(
        karas_model.keras_lstm_training
    )  # LSTM_ML_model.train_tr_model)

    # fl_server.set_ML_model_class(bClass.ModelCode(1,1))
    # fl_server.set_training_function(bClass.train_model)

    fl_server.init_parameters(
...
def split_dataset_to_training_files(dataset, client_count, path_):
    ...
    data_dir_name = do.create_dir(
        f"/{path_}/training_splits", f"{client_count}_dataset_split"
    )
